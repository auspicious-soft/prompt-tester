...
import CreatePickupPromptForm from "../utils/CraetePickupPrompt";
import PickUpPromptById from "./PickUpPomptById";
...
const PickUpPromptTemplates: React.FC = () => {
...
const [deletingPromptId, setDeletingPromptId] = useState<string | null>(null);
  ...
  const getAllPrompts = async () => {
    setLoading(true);
    try {
      const response = await getApi(`${URLS.getAllPrompts}?type=pickup`);
...
const confirmDeletePrompt = async () => {
  ...
  try {
    const response = await postApi(`${URLS.deletePrompt}?type=pickup`, {
      id: deletingPromptId,
      password,
    });
  ...
  const handleDuplicatePrompt = async () => {
    ...
    try {
      const response = await postApi(`${URLS.duplicatePrompt}?type=pickup`, {
    ...
    <div className="w-full max-w-5xl mx-auto p-4">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
        ...
        >
          ...
          >
            <option value="ALL">All Pick-up Prompts</option>
            <option value="ACTIVE">Active Pick-up App Prompts</option>
            <option value="ACTIVE_WEB">Active Pick-up Tester Prompts</option>
...
export default PickUpPromptTemplates;
